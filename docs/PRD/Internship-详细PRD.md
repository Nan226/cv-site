...
- 开场中心视觉使用 `images/Internship/主页面.jpg` 作为一张完整水彩图。
- 滚动揭开后，主页面图淡出，三张小蝴蝶卡片逐渐分裂出现。
- 三张卡片长宽比固定为 `5:7`。
...
- 页面背景保持浅蓝粉白。
- 中央出现 `images/Internship/主页面.jpg`，视觉上是一张完整的水彩主图。
- 图片动画为“唰地放大出现，再回落缩小”：
...
- 详情状态背景必须回到 `images/Internship/主页面.jpg` 的水彩氛围，不能继续暴露小蝴蝶卡片图作为大背景。
- 页面不应该跳出当前 section。
